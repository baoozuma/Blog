...
**Lemma (Vitali's Covering Lemma).** Let $(X,d)$ be a separable metric space. Suppose $\mathcal{F}$ is any collection of closed balls in $X$ such that
...
**Zorn's Lemma.** Let $S$ be an ordered set in which every chain has an upper bound. Then $S$ has at least one maximal element.
...
$$
\mathcal{P}_1 \leq \mathcal{P}_2 \iff \mathcal{P}_1 \subseteq \mathcal{P}_2, \quad \forall \mathcal{P}_1, \mathcal{P}_2 \in P_n.
$$
...
<div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', margin: '2rem 0' }}>
  <img src="/1.jpg" alt="Subcase 1: r = 2r₁, externally tangent" />
</div>
...
Then $B(x,r)$ must intersect some ball in $K_n$, otherwise $\{B(x,r)\} \cup K_n$ would be a larger disjoint collection in $P_n$, contradicting maximality of $K_n$. Let $B(y, r_1) \in K_n$ intersect $B(x,r)$.

*Subcase $B(y, r_1) \in G_n$:* Since $R2^{-n-1} < r_1 \leq R2^{-n}$, multiplying by $2$ gives $R2^{-n} < 2r_1 \leq R2^{-n+1}$, so $r < 2r_1$. For all $t \in B(x,r)$,
...
*Subcase $B(y, r_1) \notin G_n$:* There exists $B(z, r_2) \in G_i$ with $B(y, r_1) \cap B(z, r_2) \neq \varnothing$ for some $i < n$. Taking $i = n-1$, since $R2^{-n-1} < r, r_1 \leq R2^{-n}$ and $R2^{-n} < r_2 \leq R2^{-n+1}$, we have $r < r_2$ and $r_1 < r_2$.

We show $B(z, 5r_2)$ contains both $B(y, r_1)$ and $B(x,r)$:

$$
d(z, y) \leq r_1 + r_2 \leq 2r_2.
$$

For all $t \in B(y, r_1)$:

$$
d(z,t) \leq d(z,y) + d(y,t) \leq 2r_2 + r_1 \leq 3r_2 < 5r_2.
$$

So $B(y, r_1) \subseteq B(z, 5r_2)$. In addition,

$$
d(z, x) \leq d(z,y) + d(y,x) \leq 2r_2 + r_1 + r \leq 2r_2 + r_2 + r_2 = 4r_2.
$$

For all $t \in B(x,r)$:

$$
d(z,t) \leq d(z,x) + d(x,t) \leq 4r_2 + r \leq 5r_2.
$$

Hence $B(x,r) \subseteq B(z, 5r_2)$.

**Case 2: $B(x,r) \in K_n$.**

If $B(x,r) \in G_n$ we are done. Otherwise $B(x,r) \in K_n \setminus G_n$, so there exists $B(y, r_1) \in G_i$ intersecting $B(x,r)$ for some $i < n$. Taking $i = n-1$, we have $R2^{-n-1} < r \leq R2^{-n}$ and $R2^{-n} < r_1 \leq R2^{-n+1}$, so $r < r_1$. For all $t \in B(x,r)$:

$$
d(t, y) \leq d(t,x) + d(x,y) \leq r + r_1 + r < r_1 + r_1 + r_1 = 3r_1 < 5r_1.
$$
...
[1] Lawrence C. Evans and Ronald F. Gariepy, *Measure Theory and Fine Properties of Functions*, Revised Edition, CRC Press.