...
Let $\Phi: \mathbb{R}^n \to \mathbb{R}^n$ be a Lipschitz map, meaning that there is a $C > 0$ such that
...
<Lemma>
Suppose $\Phi: \mathbb{R}^n \to \mathbb{R}^n$ is a Lipschitz mapping. If $E \subseteq \mathbb{R}^n$ is Lebesgue measurable, then $\Phi(E)$ is measurable.
</Lemma>
...
<Theorem label="SVD">
Let $A \in M_{m \times n}(\mathbb{R})$. Then there exist $U \in O(m)$, $V \in O(n)$, and a diagonal matrix $\Sigma$ with nonnegative entries such that $A = U\Sigma V^T$.
</Theorem>
...
In this section, we need to define some special algebras that work quite effectively on measure.

<Definition label="pi-system">
...
<Definition label="lambda-system">
...
<Theorem label="pi-lambda Theorem">
If $\mathcal{P}$ is a $\pi$-system and $\mathcal{L}$ is a $\lambda$-system with $\mathcal{P} \subseteq \mathcal{L}$, then $\sigma(\mathcal{P}) \subseteq \mathcal{L}$.
</Theorem>
...
<Lemma>
Let $A \subseteq \mathbb{R}^n$ be a compact subset whose intersection with $\{c\} \times \mathbb{R}^{n-1}$ has $(n-1)$-dimensional measure zero for every $c \in \mathbb{R}$. Then $A$ has measure zero.
</Lemma>
...
<Lemma>
Every proper affine subspace of $\mathbb{R}^n$ has Lebesgue measure zero in $\mathbb{R}^n$.
</Lemma>
...
<Theorem label="Affine Invariant Theorem">
Let $F: \mathbb{R}^n \to \mathbb{R}^n$ be an affine map $F(x) = Ax + b$, where $A \in M_n(\mathbb{R})$ and $b \in \mathbb{R}^n$. Then
...
<Proof>
**Reduction.** Since $F(U) = A(U) + b$ and Lebesgue measure is translation invariant, $\lambda^*(A(U) + b) = \lambda^*(A(U))$. So it suffices to prove $\lambda^*(A(U)) = |\det(A)|\,\lambda^*(U)$ for all $U \subseteq \mathbb{R}^n$.
