...
tags: ['Measure Theory', 'Functional Analysis']
---

## Fatou's Lemma

<Lemma label="Fatou's Lemma">
Let $(X,\mu)$ be a measure space and let $\{f_n\}: X \to [0,+\infty]$ be $\mu$-measurable sequence of functions. Then one has
...
Let $\{f_n\}: X \to [0,+\infty]$ be increasing, $\mu$-measurable sequence. Then
...
Let $g \in L^1$ be nonnegative, and $f, \{f_n\}$ are measurable. Suppose $f_n \to f$ and $|f_n| \leq g$, for all $n$, $\mu$.a.e. Then one has
...
<Definition label="Lp Space">
Let $(X,\mathcal{A},\mu)$ be a measure space and $1 \leq p < \infty$. Let $\mathcal{M}(X)$ be the space of all $\mu$-measurable functions $f: X \to \mathbb{R}$ such that
...
<Theorem label="Riesz–Fischer">
The $L^p$ space is complete, i.e every Cauchy sequence in $L^p$ converges to an element in $L^p$.
</Theorem>
